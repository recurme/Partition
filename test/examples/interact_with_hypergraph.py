...
partitioned_hg = mtkahypar.PartitionedHypergraph(
  ...
  [0,-1,-1,1,-1,-1,2,-1])    # block IDs for each node
...
# print()
# Iterate over all nodes
block_files = [open(f"block_{i}.txt", "w") for i in range(number_of_blocks)]
hypergraph.doForAllNodes(lambda node : (
  block_files[partitioned_hg.blockID(node)].write(str(node) + "\n"),
  # block_files[partitioned_hg.blockID(node)].write(str(partitioned_hg.blockID(node)) + "\n"),
  # block_files[partitioned_hg.blockID(node)].write(str(partitioned_hg.numIncidentCutEdges(node)) + "\n")
))
